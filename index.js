...
module.exports = postcss.plugin("postcss-remtorpx", options => {
  const opts = Object.assign({}, defaults, options);
  const remRegex = require("./lib/pixel-unit-regex")(opts.beforeUnit);
  ...
  return css => {
    ...
    css.walkDecls((decl, i) => {
      if (
        decl.value.indexOf(opts.beforeUnit) === -1 ||
        unsatisfyPropList(decl.prop) ||
        blacklistedSelector(opts.selectorBlackList, decl.parent.selector)
      )
        return;

      const value = decl.value.replace(remRegex, pxReplaceFunc);
    ...
    if (opts.mediaQuery) {
      css.walkAtRules("media", rule => {
        if (rule.params.indexOf(opts.beforeUnit) === -1) return;
        rule.params = rule.params.replace(remRegex, pxReplaceFunc);
      });
    }
  };
});
